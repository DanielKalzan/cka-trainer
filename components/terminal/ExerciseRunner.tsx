...
export default function ExerciseRunner({ domainId, exerciseId }: ExerciseRunnerProps) {
  ...
  return <LiveRunner key={exerciseId} exercise={exercise} />;
}

/** Grading is async: the check request goes over the terminal's WebSocket to
 *  the bridge, which grades the exercise's real cluster state. */
